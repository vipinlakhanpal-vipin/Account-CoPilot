...
- **Scheduled sessions** follow `ENGINE.md`: claim a job (`scripts/engine_jobs.mjs`), verify or discover with web search, add companies with `scripts/add_companies.mjs`, then apply, recompute and mark the job done. The revenue queue skips companies that already have `profile["Revenue check"]`, so cloud sessions don't redo work. For the routine to run, GitHub must be connected to Claude and `NEXT_PUBLIC_SUPABASE_URL` / `SUPABASE_SECRET_KEY` set in its environment.
